...
class GameTests: XCTestCase {
    ...
    func testActivePlayer() {
        do {
            XCTAssertTrue(userGame?.activePlayer == .x, "currently active player is \(String(describing: userGame?.activePlayer))")
            try userGame?.makeMark(at: (0, 0))
            XCTAssertTrue(userGame?.activePlayer == .o, "currently active player is \(String(describing: userGame?.activePlayer))")
            try userGame?.makeMark(at: (1, 1))
            XCTAssertTrue(userGame?.activePlayer == .x, "currently active player is \(String(describing: userGame?.activePlayer))")
            try userGame?.makeMark(at: (0, 1))
            XCTAssertTrue(userGame?.activePlayer == .o, "currently active player is \(String(describing: userGame?.activePlayer))")
        ...
        XCTAssertNil(userGame?.activePlayer, "currently active player is \(String(describing: userGame?.activePlayer))")
